...
open_quizz_db_data = (
   ...
   # ("Bande_dessinnée", "Tintin", "https://www.kiwime.com/oqdb/files/2124627384/OpenQuizzDB_124/openquizzdb_124.json"),
...
def generate_json_file(categorie, titre, url):
    ...
    for quizz_difficulty, quizz_data in all_quizz.items():
        out_filename = get_quizz_filename(categorie, titre, quizz_difficulty)
        print(out_filename)
        out_questionnaire_data["difficulte"] = quizz_difficulty
...
""" for i in range(len(open_quizz_db_data)):
    try:
        generate_json_file(open_quizz_db_data[i][0], open_quizz_db_data[i][1], open_quizz_db_data[i][2])
    except:
        print(open_quizz_db_data[i], i)
        exit()
    #else:
        #print(open_quizz_db_data[i])
    print() """